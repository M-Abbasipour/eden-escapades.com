...
function PageMain() {
    ...
    $home['title'] = (!empty($title) ? $title : $LNG['title_profile']).' - '.realName(htmlspecialchars($_GET['u']), $feed->profile_data['first_name'], $feed->profile_data['last_name'], 1).' - '.$settings['title'];
	...
	if($_GET['r'] == 'edit' && $feed->page_data['by'] == $feed->id) {
		...
		$TMPL['edit_url'] = '&name='.$feed->page_data['name'].'&r=edit';
		$TMPL['delete_url'] = '&name='.$feed->page_data['name'].'&r=delete';
		...
		if(!empty($_POST)) {
			...
			if($message[0]) {
				...
				header("Location: ".$CONF['url']."/index.php?a=page&name=".$message[1]);
			}
		}
	}
