...
function PageMain() {
	...
	if(isset($_GET['facebook']) && $settings['fbappid']) {
		...
		if($TMPL['registerMsg'] == 1) {
			...
			header("Location: ".$CONF['url']."/index.php?a=feed&message=welcome");
		}
	}
	...
	if(isset($_POST['register'])) {
		...
		if($TMPL['registerMsg'] == 1) {
			if($settings['mail']) {
				sendMail($_POST['email'], sprintf($LNG['welcome_mail'], $settings['title']), sprintf($LNG['user_created'], $settings['title'], $_POST['username'], $_POST['password'], $_POST['gender'], $CONF['url'], $settings['title']), $CONF['email']);
			}
			header("Location: ".$CONF['url']."/index.php?a=feed&message=welcome");
		}
	}
	...
	if(isset($_SESSION['username']) && isset($_SESSION['password']) || isset($_COOKIE['username']) && isset($_COOKIE['password'])) {
		...
		if($verify['username']) {
			header("Location: ".$CONF['url']."/index.php?a=home");
		}
	}
