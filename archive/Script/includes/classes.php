...
function menu($user) {
	...
	if($user !== false) {
		...
		*/
		...
		return '<a href="'.$CONF['url'].'/index.php?a=welcome" rel="loadpage"></a>'.$admin_url;
	}
}
function chatSmiles() {
	global $CONF;
	...
	foreach($smiles as $code => $smile) {
		$output .= '<a onclick="addSmile(\''.addslashes($code).'\')" title="'.$code.'"><img src="'.$CONF['url'].'/'.$CONF['theme_url'].'/images/icons/emoticons/'.$smile.'" height="14" width="14"></a>';
	}
	return $output;
}
...
class register {
	...
	function verify_if_email_exists($type = null) {
		...
		if($type) {
			$query = sprintf("SELECT `username`, `password` FROM `users` WHERE `email` = '%s' AND `suspended` != 2", $this->db->real_escape_string(strtolower($this->email)));
		} else {
			$query = sprintf("SELECT `email` FROM `users` WHERE `email` = '%s' AND `suspended` != 2", $this->db->real_escape_string(strtolower($this->email)));
		}
		$result = $this->db->query($query);
	...
	function query() {
		...
		$query = sprintf("INSERT INTO `users` (`username`, `password`, `first_name`, `last_name`, `email`, `date`, `image`, `privacy`, `cover`, `verified`, `online`, `salted`, `suspended`, `ip`, `notificationl`, `notificationc`, `notifications`, `notificationd`, `notificationf`, `notificationg`, `notificationx`, `notificationp`, `email_comment`, `email_like`, `email_new_friend`, `email_page_invite`, `email_group_invite`, `sound_new_notification`, `sound_new_chat`, `gender`) VALUES ('%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', 'default.png', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s');", $this->db->real_escape_string(strtolower($this->username)), md5($this->db->real_escape_string($this->password)), $this->db->real_escape_string($this->first_name), $this->db->real_escape_string($this->last_name), $this->db->real_escape_string($this->email), date("Y-m-d H:i:s"), ($this->profile_image ? $this->profile_image : 'default.png'), 1, $this->verified, time(), $salt, $suspended, $this->db->real_escape_string(getUserIp()), 1, 1, 1, 1, 1, 1, 1, 1, $this->email_comment, $this->email_like, $this->email_new_friend, $this->email_page_invite, $this->email_group_invite, 1, 1, $this->gender);
		$this->db->query($query);
...
class logIn {
	...
	function in() {
		...
		if($user == 1) {
			...
			// Redirect the user to his personal profile
			header("Location: ".$this->url."/index.php?a=home");
		} else {
...
class loggedInAdmin {
	...
	function query() {
		...
		if($result->num_rows == 0) {
			...
			if(filter_var($this->db->real_escape_string($this->username), FILTER_VALIDATE_EMAIL)) {
				$query = sprintf("SELECT * FROM `users` WHERE `email` = '%s' AND `password` = '%s' AND `user_group` = 1 AND `suspended` = 0", $this->db->real_escape_string($this->username), $this->db->real_escape_string($this->password));
			} else {
				$query = sprintf("SELECT * FROM `users` WHERE `username` = '%s' AND `password` = '%s' AND `user_group` = 1 AND `suspended` = 0", $this->db->real_escape_string($this->username), $this->db->real_escape_string($this->password));
			}
		...
		if($data['token_id'] == $_SESSION['token_id']) {
			...
			if($x !== 1) {
				...
				if($table == 'admin') {
					...
					$stmt = $this->db->prepare("UPDATE `$table` SET `password` = md5(?) WHERE `username` = ?");
					$stmt->bind_param("ss", $data['password'], $_SESSION['usernameAdmin']);
...
class recover {
	...
	function checkUser() {
		...
		if(filter_var($this->db->real_escape_string($this->username), FILTER_VALIDATE_EMAIL)) {
			$query = sprintf("SELECT `username`,`email` FROM `users` WHERE `email` = '%s' AND `suspended` = 0", $this->db->real_escape_string(strtolower($this->username)));
		} else {
			$query = sprintf("SELECT `username`,`email` FROM `users` WHERE `username` = '%s' AND `suspended` = 0", $this->db->real_escape_string(strtolower($this->username)));
		}
	...
	function getUsers($start, $type = null) {
		...
		if($loadmore) {
			$users .= '<div class="admin-load-more"><div class="message-container" id="more_users">
					<div class="load_more"><a onclick="manage_the('.$last.', 0, '.saniscape($type).')" id="load-more">'.$LNG['view_more_messages'].'</a></div>
				</div></div>';
		}
	...
	function getReports($start) {
		...
		if($loadmore) {
			$users .= '<div class="admin-load-more"><div class="message-container" id="more_reports">
					<div class="load_more"><a onclick="manage_the('.$last.', 1)" id="load-more">'.$LNG['view_more_messages'].'</a></div>
				</div></div>';
		}
...
class feed {
	...
	function getMessages($query, $type, $typeVal) {
		...
		foreach($rows as $row) {
			...
			$shared_title = $sharedMedia = $sharedContent = $group_title = $this->page_data = '';
			if($row['group']) {
				$dataType = 2;
				...
				if($this->is_post_page) {
					...
					if($group['privacy'] && $this->is_admin == 0) {
						...
						if(!$this->groupPermission($group, $this->group_member_data)) {
							header('Location: '.$this->url.'/index.php?a=group&name='.$group['name']);
						}
					}
			...
			if($row['type'] == 'shared') {
				...
				$shared_url = ($getPage['name'] ? $this->url.'/index.php?a=page&name='.$getPage['name'] : $this->url.'/index.php?a=profile&u='.$shared['username']);
				$shared_ttl = ($getPage['name'] ? $getPage['title'] : realName($shared['username'], $shared['first_name'], $shared['last_name']));
				
				$shared_title = ' '.sprintf($LNG['shared_title'], $shared_url, $shared_ttl, $this->url.'/index.php?a=post&m='.$row['value']);
			}
			...
			$profile_url = ($row['idu'] == $this->page_data['by'] ? $this->url.'/index.php?a=page&name='.$this->page_data['name'] : $this->url.'/index.php?a=profile&u='.$row['username']);
			$profile_img = ($row['idu'] == $this->page_data['by'] ? $this->url.'/thumb.php?src='.$this->page_data['image'] : $this->url.'/thumb.php?src='.$row['image']);
			$profile_name = ($row['idu'] == $this->page_data['by'] ? $this->page_data['title'] : realName($row['username'], $row['first_name'], $row['last_name']));
			$page = $this->page_data['id'] ? 1 : 0;
			$row['idu'] = $this->page_data['id'] ? $this->page_data['id'] : $row['idu'];
			$messages .= '
			<div class="message-container last-message" id="message'.$row['id'].'" data-filter="'.str_replace('\'', '', $typeVal) .'" data-last="'.$row['id'].'" data-username="'.$this->profile.'" data-type="'.$dataType.'" data-userid="'.$row['uid'].'">
				<div class="message-content">
					<div class="message-inner">
						<div class="message-avatar" id="avatar'.$row['id'].'">
							<a href="'.$profile_url.'" rel="loadpage">
								<img onmouseover="profileCard('.$row['idu'].', '.$row['id'].', 0, 0, '.$page.');" onmouseout="profileCard(0, 0, 0, 1, '.$page.');" onclick="profileCard(0, 0, 1, 1, '.$page.');" src="'.$profile_img.'&t=a&w=50&h=50">
							</a>
						</div>
						<div class="message-top">
							'.$menu.'
					...
					'.($sharedMedia ? $sharedMedia : $this->getType($row['type'], $row['value'], $row['id'])).$po.'
					...
					<div class="message-comment-box-container" id="comment_box_'.$row['id'].'"'.$style.'>
						<div class="message-reply-avatar">
							'.((!empty($this->user)) ? '<img src="'.$this->url.'/thumb.php?src='.($this->id == $this->page_data['by'] ? $this->page_data['image'] : $this->user['image']).'&t=a&w=50&h=50">' : '').'
						</div>
	...
	function getFeed($start, $value, $from = null) {
		...
		$this->listTypes = $this->listTypes($this->friendsList, $this->pages);
		$this->listDates = $this->listDates($this->friendsList, $this->pages);
		...
		if(in_array($value, $this->listTypes)) {
			...
			$date = sprintf("AND extract(YEAR_MONTH from `time`) = '%s'", $this->db->real_escape_string($value));
		}
		
		// Get the liked page's feed if any
		if(!empty($this->pages)) {
			$pages = sprintf("OR (messages.page IN (%s) %s AND messages.uid = users.idu AND users.suspended = 0 %s %s)",$this->db->real_escape_string($this->pages), $date.$type, $start, $from);
		}
	...
	function getProfile($start, $value, $from = null) {
		...
		if(!empty($profile['idu'])) {
			...
			$query = sprintf("SELECT * FROM messages, users WHERE messages.uid = '%s' %s AND messages.group = 0 AND messages.page = 0 AND messages.uid = users.idu %s %s %s ORDER BY messages.id DESC LIMIT %s", $this->db->real_escape_string($profile['idu']), $type.$date, $public, $start, $from, ($this->per_page + 1));
		...
		$cover = '<div class="twelve columns">
					<div class="cover-container">
						<div class="cover-content">
							<a onclick="gallery(\''.$group['cover'].'\', \''.$group['id'].$group['title'].'\', \'covers\')" id="'.$group['cover'].'"><div class="cover-image" style="background-position: center; background-image: url('.$this->url.'/thumb.php?src='.$group['cover'].'&w=900&h=200&t=c)">
							</div></a>
	...
	function sidebarPageInfo($page) {
		...
		$extra = ($page['posts'] ? $LNG['admins_posts'] : $LNG['members_posts']);
		
		$rows = array(
			// $LNG['created_on']	=> $birthdate,
			$LNG['likes'] => array('like', ($this->page_stats['total'] > 0 ? '<a href="'.$this->url.'/index.php?a=page&name='.$this->page_data['name'].'&r=likes" rel="loadpage">'.$this->page_stats['total'].' '.$LNG['people'].'</a>' : '')),
		...
		$info = '<div class="sidebar-container widget-group-info"><div class="sidebar-content"><div class="sidebar-header">'.$LNG['profile_about'].''.(($this->page_data['by'] == $this->id) ? ' <span class="sidebar-header-link"><a href="'.$this->url.'/index.php?a=page&name='.$page['name'].'&r=edit" rel="loadpage">'.$LNG['admin_ttl_edit'].'</a></span>' : '').'</div>';
	...
	function fetchPage($page) {
		...
		$cover = '<div class="twelve columns">
					<div class="cover-container">
						<div class="cover-content">
							<a onclick="gallery(\''.$page['cover'].'\', \''.$page['id'].$page['name'].'\', \'covers\')" id="'.$page['cover'].'"><div class="cover-image" style="background-position: center; background-image: url('.$this->url.'/thumb.php?src='.$page['cover'].'&w=900&h=200&t=c)">
							</div></a>
							<div class="cover-description">
								<div class="cover-avatar-content">
									<div class="cover-avatar">
										<a onclick="gallery(\''.$page['image'].'\', \''.$page['id'].$page['name'].'\', \'avatars\')" id="'.$page['image'].'"><span id="avatar'.$page['id'].$page['name'].'"><img src="'.$this->url.'/thumb.php?src='.$page['image'].'&t=a&w=150&h=150"></span></a>
									</div>
								</div>
	...
	function fetchProfile($profile) {
		...
		$cover = '<div class="twelve columns">
					<div class="cover-container">
						<div class="cover-content">
							<a onclick="gallery(\''.$profile['cover'].'\', \''.$profile['idu'].$profile['username'].'\', \'covers\')" id="'.$profile['cover'].'"><div class="cover-image" style="background-position: center; background-image: url('.$this->url.'/thumb.php?src='.$profile['cover'].'&w=900&h=200&t=c)">
							</div></a>
							<div class="cover-description">
								<div class="cover-avatar-content">
									<div class="cover-avatar">
										<a onclick="gallery(\''.$profile['image'].'\', \''.$profile['idu'].$profile['username'].'\', \'avatars\')" id="'.$profile['image'].'"><span id="avatar'.$profile['idu'].$profile['username'].'"><img src="'.$this->url.'/thumb.php?src='.$profile['image'].'&t=a&w=150&h=150"></span></a>
									</div>
								</div>
								<div class="cover-buttons">
									'.$this->coverButtons(0).'
	...
	function profileCard($data, $type) {
		...
		if($type) {
			$name = $data['title'];
			$url = $this->url.'/index.php?a=page&name='.$data['name'];
			$likes = $this->countLikes($data['id'], 2);
			...
			$url = $this->url.'/index.php?a=profile&u='.$data['username'];
		...
		$card = '
			<div class="profile-card-cover">
				<img src="'.$this->url.'/thumb.php?src='.$data['cover'].'&w=300&h=100&t=c">
				<div class="profile-card-info">
					<a href="'.$url.'" rel="loadpage"><span id="author'.$data['idu'].$data['username'].'"></span><span id="time'.$data['idu'].$data['username'].'"></span><div class="cover-username">'.$name.''.((!empty($data['verified'])) ? '<img src="'.$this->url.'/'.$CONF['theme_url'].'/images/icons/verified.png" title="'.$LNG['verified_page'].'" height="16" width="16">' : '').'</div></a>
				</div>
			</div>
			<div class="profile-card-avatar">
				<a href="'.$url.'"><img src="'.$this->url.'/thumb.php?src='.$data['image'].'&t=a&w=112&h=112"></a>
			</div>
	...
	function sidebarProfileInfo($profile) {
		...
		$rows = array(
            ...
            $LNG['ttl_gender']			=> array($profile['gender'], $gender),
		...
		if(!$type) {
			...
			if($type) {
				...
				if(!$for) {
					...
					if(empty($likes) && empty($comments) && empty($shares) && empty($friends) && empty($chats) && empty($pages) && empty($groups) && empty($pokes) || $type == 2) {
						$all = 1;
					}
				}
				...
				if($all) {
					...
					if($for != 1) {
						...
						if($pn) {
							$checkPokes = $this->db->query(sprintf("SELECT * FROM `notifications`,`users` WHERE `notifications`.`from` = `users`.`idu` AND `notifications`.`to` = '%s' and `notifications`.`from` <> '%s' AND `notifications`.`type` = '8' ORDER BY `notifications`.`id` DESC LIMIT %s", $this->db->real_escape_string($this->id), $this->db->real_escape_string($this->id), $limit));
						...
						if($xn) {
							$checkPages = $this->db->query(sprintf("SELECT `notifications`.`id`, `notifications`.`from`, `notifications`.`to`, `notifications`.`parent`, `notifications`.`type`, `notifications`.`read`, `notifications`.`time`, `users`.`username`, `users`.`first_name`, `users`.`last_name`, `users`.`image`, `pages`.`title`, `pages`.`name` FROM `notifications`,`users`,`pages` WHERE `notifications`.`from` = `users`.`idu` AND `notifications`.`to` = '%s' and `notifications`.`from` <> '%s' AND `notifications`.`type` = '9' AND `pages`.`id` = `notifications`.`parent` ORDER BY `notifications`.`id` DESC LIMIT %s", $this->db->real_escape_string($this->id), $this->db->real_escape_string($this->id), $limit));
					...
					if($for == 2) {
						// Verify for the notifications page
						if(empty($likes) && empty($comments) && empty($shares) && empty($friends) && empty($chats) && empty($birthdays) && empty($pages) && empty($groups) && empty($pokes)  && empty($pictures)) {
			...
			foreach($pictures as $picture) {
				$pictures[$g]['event'] = 'picture';
				$g++;
			}
			
			$array = array_merge($likes, $comments, $shares, $friends, $chats, $birthdays, $pages, $groups, $pokes, $pictures);
			...
			foreach($array as $value) {
				...
				if($value['event'] == 'like') {
					...
					$events .= '<div class="notification-image"><a href="'.$this->url.'/index.php?a=profile&u='.$value['username'].'" rel="loadpage"><img class="notifications" src='.$this->url.'/thumb.php?src='.$value['image'].'&t=a&w=50&h=50"></a></div><div class="notification-text">'.sprintf($LNG['new_page_notification'], $this->url.'/index.php?a=profile&u='.$value['username'], realName($value['username'], $value['first_name'], $value['last_name']), $this->url.'/index.php?a=page&name='.$value['name'], $value['title']).'.<br><img src="'.$this->url.'/'.$CONF['theme_url'].'/images/icons/n_page.png" width="16" height="16"><span class="timeago'.$b.'" title="'.$time.'">'.$time.'</span></div>';
	...
	function comments($query, $values = null) {
		...
		if($result = $this->db->query($query)) {
			...
			foreach($rows as $comment) {
				...
				$verify = $this->verifyLike($comment['id'], 1);
				...
				if($this->username) {
					...
					if($this->username == $comment['username'] || $values['owner']) { // If it's current username is the same with the current author
						$item .= '<div class="message-menu-row" onclick="deleteModal('.$comment['id'].', 0)">'.$LNG['delete'].'</div>';
					}
				...
				if($this->page_data['by'] == $comment['idu']) {
				...
				<div class="message-reply-container" id="comment'.$comment['id'].'">
					'.$menu.'
					<div class="message-reply-avatar">
						<a href="'.$this->url.'/index.php?a=profile&u='.$comment['username'].'" rel="loadpage"><img onmouseover="profileCard('.$comment['idu'].', '.$comment['id'].', 1, 0, 0)" onmouseout="profileCard(0, 0, 1, 1, 0);" onclick="profileCard(0, 0, 1, 1, 0);" src="'.$this->url.'/thumb.php?src='.$comment['image'].'&t=a"></a>
					</div>
					<div class="message-reply-message">
						<span class="message-reply-author"><a href="'.$this->url.'/index.php?a=profile&u='.$comment['username'].'" rel="loadpage">'.$name.'</a></span>: '.$this->parseMessage($comment['message']).'
					</div>
			...
			if($loadmore && $this->c_per_page) {
				$load = '<div class="load-more-comments" id="more_comments_'.htmlentities($values['id'], ENT_QUOTES).'"><a onclick="loadComments('.$message_id.', '.$rows[1]['id'].', '.($values['start'] + $this->c_per_page).')">'.$LNG['view_more_comments'].'</a></div>';
			}
	...
	function like($id, $type, $action = null) {
		...
		if($type == 1) {
			...
			$likes = $this->getLikers($child, ($action == 1 ? $post['likes'] + 1 : $post['likes'] - 1), 1);
		} else {
			$parent = $post['id'];
			$child = 0;
			...
			$likes = $this->getLikers($parent, ($action == 1 ? $post['likes'] + 1 : $post['likes'] - 1));
		}
		...
		if($action > 0 && empty($post['page'])) {
			...
			if($this->email_like) {
				...
				if($post['email_like'] && ($this->id !== $post['idu'])) {
					// Send e-mail
					sendMail($post['email'], sprintf($email_title, $this->username), sprintf($email_content, realName($post['username'], $post['first_name'], $post['last_name']), $this->url.'/index.php?a=profile&u='.$this->username, $this->username, $email_url, $this->title, $this->url.'/index.php?a=settings&b=notifications'), $this->email);
				}
			}
		} else {
		...
		// Return the output
		return json_encode(array('value' => $value, 'count' => $likes));
	}
	...
	function getLikers($id, $count, $type = null) {
		...
		if($this->l_per_post && $count > 0) {
			$query = sprintf("SELECT * FROM `likes`,`users` WHERE `post` = '%s' AND `likes`.`type` = 0 AND `likes`.`by` = `users`.`idu` ORDER BY `likes`.`id` DESC LIMIT %s", $this->db->real_escape_string($id), $this->db->real_escape_string($this->l_per_post));
	...
	function parseMessage($message) {
		...
		// Parse links
		$parseUrl = preg_replace_callback('/(?i)\b((?:https?:\/\/|www\d{0,3}[.]|[a-z0-9.\-]+[.][a-z]{2,4}\/)(?:[^\s()<>]+|\(([^\s()<>]+|(\([^\s()<>]+\)))*\))+(?:\(([^\s()<>]+|(\([^\s()<>]+\)))*\)|[^\s`!()\[\]{};:\'".,<>?´ªìîëí]))/', "parseCallback", $message);
	...
	function getType($type, $value, $id) {
		...
		switch($type) {
			...
			case "picture":
				...
				if(count($images) == 1) {
					$result .= '<div class="message-type-image event-picture"><a onclick="gallery(\''.$images[0].'\', '.$id.', \'media\')" id="'.$images[0].'"><img src="'.$this->url.'/thumb.php?src='.$images[0].'&w=650&h=300&t=m"></a>';
				} elseif(count($images) == 2) {
					$result .= '<div class="message-type-image event-picture"><div class="image-container-padding">';
					foreach($images as $image) {
						$result .= '<a onclick="gallery(\''.$image.'\', '.$id.', \'media\')" id="'.$image.'"><div class="image-thumbnail-container-half"><div class="image-thumbnail"><img src="'.$this->url.'/thumb.php?src='.$image.'&w=300&h=300&t=m"></div></div></a>';
					}
					$result .= '</div>';
				} elseif(count($images) == 3) {
					$result .= '<div class="message-type-image event-picture"><div class="image-container-padding">';
					foreach($images as $image) {
						$result .= '<a onclick="gallery(\''.$image.'\', '.$id.', \'media\')" id="'.$image.'"><div class="image-thumbnail-container"><div class="image-thumbnail"><img src="'.$this->url.'/thumb.php?src='.$image.'&w=200&h=200&t=m"></div></div></a>';
					}
					$result .= '</div>';
					...
					foreach($images as $image) {
						if($i == 1) {
							$result .= '<a onclick="gallery(\''.$image.'\', '.$id.', \'media\')" id="'.$image.'"><div class="fake"><div class="image-thumbnail"><img src="'.$this->url.'/thumb.php?src='.$image.'&w=650&h=300&t=m"></div></div></a>';
						} else {
							// If there are more than 4 images, hide them
							if($i > 4) {
								$result .= '<a onclick="gallery(\''.$image.'\', '.$id.', \'media\')" id="'.$image.'" style="display: none;"></a>';
							} else {
								$result .= '<a onclick="gallery(\''.$image.'\', '.$id.', \'media\')" id="'.$image.'"><div class="image-thumbnail-container"><div class="image-thumbnail">'.($i == 4 && count($images) > 4 ? '<span class="image-thumbnail-text">+ '.(count($images)-4).'</span>' : '').'<img src="'.$this->url.'/thumb.php?src='.$image.'&w=200&h=200&t=m"></div></div></a>';
							}
						}
						$i++;
					}
	...
	function delete($id, $type) {
		...
		if($type == 0) {
			...
			$ownership = $this->db->query(sprintf("SELECT `comments`.`uid` as `cuid`, `messages`.`uid` as `muid` FROM `comments`, `messages` WHERE `comments`.`id` = '%s' AND `comments`.`mid` = `messages`.`id`", $this->db->real_escape_string($id)));
			$verify = $ownership->fetch_assoc();
			
			// If the logged-in user is the message owner
			if($this->id == $verify['muid']) {
	...
	function sidebarSearch() {
		...
		// Start the output
		$row = array('tag', 'groups', 'pages');
		$link = '<div class="sidebar-container widget-search"><div class="sidebar-content"><div class="sidebar-header">'.$LNG['title_search'].'</div>';
		if(!empty($_GET['q'])) {
			$class = ' sidebar-link-active';
		}
		$link .= '<div class="sidebar-link'.$class.'"><a href="'.$this->url.'/index.php?a='.$_GET['a'].'&q='.htmlspecialchars($_GET['q'].$_GET['tag'].$_GET['groups'].$_GET['pages'], ENT_QUOTES, 'UTF-8').'" rel="loadpage">'.$LNG["sidebar_people"].'</a></div>';

		foreach($row as $type) {
			$class = '';
			$url = '&'.$type.'='.htmlspecialchars($_GET['q'].$_GET['tag'].$_GET['groups'].$_GET['pages'], ENT_QUOTES, 'UTF-8');
			if(!empty($_GET[$type])) {
				$class = ' sidebar-link-active';
			}
	...
	function sidebarAge($bold) {
		...
		$link .= '<div class="sidebar-link'.$class.'"><a href="'.$this->url.'/index.php?a='.$_GET['a'].'&q='.htmlspecialchars($_GET['q'], ENT_QUOTES, 'UTF-8').((!empty($_GET['filter'])) ? '&filter='.htmlspecialchars($_GET['filter'], ENT_QUOTES, 'UTF-8') : '').'" rel="loadpage">'.$LNG["all_ages"].'</a></div>';
		foreach($ages as $age) {
			// Split the ages
			$between = explode('-', $age);
			...
			// Output the links
			$link .= '<div class="sidebar-link'.$class.'"><a href="'.$this->url.'/index.php?a='.$_GET['a'].'&q='.htmlspecialchars($_GET['q'], ENT_QUOTES, 'UTF-8').'&age='.$age.((!empty($_GET['filter'])) ? '&filter='.htmlspecialchars($_GET['filter'], ENT_QUOTES, 'UTF-8') : '').'" rel="loadpage">'.$between[1].' - '.$between[0].'</a></div>';
		}
		$link .= '</div></div>';
		return $link;
	}
	...
	function sidebarNotifications($bold) {
		...
		// Start the output
		$row = array('likes', 'comments', 'shared', 'friendships', 'pages', 'groups', 'chats', 'pokes', 'birthdays');
	...
	function listTypes($friends = null, $pages = null) {
		if($friends == false && $pages == false) {
			return false;
			...
			if($pages) {
				$pages = sprintf("OR (`messages`.`uid` = `users`.`idu` AND `messages`.`page` IN (%s) AND `users`.`suspended` = 0)", $this->db->real_escape_string($this->pages));
			}
			$query = sprintf("SELECT DISTINCT `messages`.`type` FROM `messages`, `users` WHERE (`messages`.`uid` = `users`.`idu` AND `messages`.`group` = 0 AND `messages`.`page` = 0 AND `messages`.`uid` IN (%s) AND `messages`.`public` <> 0 AND `users`.`suspended` = 0) %s", $this->db->real_escape_string($friends), $pages);
		}
		$result = $this->db->query($query);
	...
	function listDates($friends = null, $pages = null) {
		if($friends == false && $pages == false) {
			return false;
		} elseif($friends == 'profile') {
			// If the user that views the profile is not the owner
		...
		} elseif($friends || $pages) {
			if($pages) {
				$pages = sprintf("OR (`messages`.`uid` = `users`.`idu` AND `messages`.`page` IN (%s) AND `users`.`suspended` = 0)", $this->db->real_escape_string($pages));
			}
			
			$query = sprintf("SELECT DISTINCT extract(YEAR_MONTH from `messages`.`time`) AS dates FROM `messages`, `users` WHERE (`messages`.`uid` = `users`.`idu` AND `messages`.`group` = 0 AND `messages`.`uid` IN (%s) AND `messages`.`public` <> 0 AND `users`.`suspended` = 0) %s ORDER BY `messages`.`time` DESC", $this->db->real_escape_string($friends), $pages);
		}
	...
	function sidebarFriends($type, $for) {
		...
		if($rows[1] > 0) {			
			if($for == 0) {
				// If the request is being made from the Profile page and the user is logged-in
				if($this->profile_data['idu'] && $this->id && $this->id !== $this->profile_data['idu']) {
					// Store the IDs into an array to compare them for Mutual friendships
					foreach($rows[0] as $user) {
						$friends_two[] = $user['idu'];
					}
					// Get the logged-in user friends list
					$friends_one = explode(',', $this->friends);

					// Count the common friends ids
					$mutual = count(array_intersect($friends_one, $friends_two));
					$mutual_text = '<span class="sidebar-header-link"><a href="'.$this->url.'/index.php?a=profile&u='.((!empty($this->profile)) ? $this->profile : $this->username).'&r=friends" rel="loadpage">'.$LNG['mutual'].' <span class="sidebar-header-light">('.$mutual.')</span></a></span>';
				}	

				$i = 0;
				$output = '<div class="sidebar-container widget-friends"><div class="sidebar-content"><div class="sidebar-header"><a href="'.$this->url.'/index.php?a=profile&u='.((!empty($this->profile)) ? $this->profile : $this->username).'&r=friends" rel="loadpage">'.$LNG['friends'].' <span class="sidebar-header-light">('.$rows[1].')</span></a>'.$mutual_text.'</div><div class="sidebar-padding">';
	...
	function sidebarPages($visible = null) {
		...
		// Select the pages
		$query = $this->db->query(sprintf("SELECT * FROM `pages` WHERE `pages`.`by` = '%s' ORDER BY `time` DESC", $this->db->real_escape_string($this->id)));
		$row = array();
		...
		$output = '<div class="sidebar-container widget-pages"><div class="sidebar-content"><div class="sidebar-header"><a href="'.$this->url.'/index.php?a=page" rel="loadpage">'.$LNG['pages'].'</a></span></div>';

		if(!$visible) {
			$output .= '<div class="sidebar-link"><a href="'.$this->url.'/index.php?a=page" rel="loadpage"><img src="'.$this->url.'/'.$CONF['theme_url'].'/images/icons/plus.png" width="24" height="24">'.$LNG['create_page'].'</a></div>';
		}
		...
		if($row) {
			...
			foreach($row as $page) {
				$menulist = '
				<a href="'.$this->url.'/index.php?a=page&name='.$page['name'].'&r=edit" rel="loadpage"><div class="message-menu-row">'.$LNG['edit'].'</div></a>
				<div class="message-menu-divider"></div>
				<a href="'.$this->url.'/index.php?a=page&name='.$page['name'].'&r=delete" rel="loadpage"><div class="message-menu-row">'.$LNG['delete'].'</div></a>';
				...
				$notifications = $this->pageActivity(0, $page['id']);
				
				$output .= '<div class="sidebar-link sidebar-page" id="page-'.$page['id'].'"'.$hidden.'><a href="'.$this->url.'/index.php?a=page&name='.$page['name'].'" rel="loadpage"><img src="'.$this->url.'/thumb.php?src='.$page['image'].'&w=48&h=48&t=a" width="24" height="24">'.($notifications ? '<span class="admin-notifications-number sidebar-notifications-number">'.$notifications.'</span>' : '').''.$page['title'].'</a><div class="sidebar-settings-container" onclick="messageMenu('.$page['id'].', 3)"><div class="settings_btn sidebar-settings'.($visible ? '' : ' s-settings-hidden').'"></div></div></div>';
	...
	function sidebarGroups($visible = null) {
		...
		if(!$visible) {
			$output .= '<div class="sidebar-link"><a href="'.$this->url.'/index.php?a=group" rel="loadpage"><img src="'.$this->url.'/'.$CONF['theme_url'].'/images/icons/plus.png" width="24" height="24">'.$LNG['create_group'].'</a></div>';
		}
	...
	function sidebarInput($type) {
		...
		if($type == 1) {
			...
			$url = 'index.php?a='.$name.'&name='.($type == 2 ? $this->page_data['name'] : $this->group_data['name']).'&friends=';
			$placeholder = $LNG['search_in_friends'];
			$value = (!empty($_GET['friends']) ? $_GET['friends'] : '');
		}
		
		
		$output = '<div class="sidebar-container widget-'.$class.'"><div class="sidebar-content"><div class="sidebar-header">'.$title.'</div><div class="sidebar-inner"><input type="text" name="search-sidebar" id="'.$class.'" class="search-sidebar" onkeydown="if(event.keyCode==13){searchFriends(\''.$url.'\', '.$type.')}" placeholder="'.$placeholder.'" value="'.$value.'"><div id="search-sidebar-btn" onclick="searchFriends(\''.$url.'\', '.$type.');"></div></div></div></div>';
			...
			if(!$status) {
				...
				if($this->email_page_invite) {
					...
					if($row['email_page_invite'] && ($this->id !== $row['idu'])) {
						...
						// Send e-mail
						sendMail($row['email'], sprintf($LNG['ttl_page_invite'], $this->username), sprintf($LNG['page_invite'], realName($row['username'], $row['first_name'], $row['last_name']), $this->url.'/index.php?a=profile&u='.$this->username, $this->username, $this->url.'/index.php?a=page&name='.$this->page_data['name'], $this->page_data['title'], $this->title, $this->url.'/index.php?a=settings&b=notifications'), $this->email);
					}
				}
			}
		} else {
								...
								<div class="message-inner">
									...
									<div class="message-top">
										<div class="message-author" id="author'.$row['idu'].'">
											<a href="'.$this->url.'/index.php?a=profile&u='.$row['username'].'" rel="loadpage">'.$row['username'].'</a>'.((!empty($row['verified'])) ? '<span class="verified-small"><img src="'.$this->url.'/'.$CONF['theme_url'].'/images/icons/verified.png" title="'.$LNG['verified_page'].'"></span>' : '').'
										</div>
										<div class="message-time">
											'.realName(null, $row['first_name'], $row['last_name']).''.((!empty($row['location']) && $row['private'] == 0) ? ' ('.$row['location'].')' : '&nbsp;').' 
										</div>
	...
	function sidebarFriendsActivity($limit, $type = null) {
		...
		$checkMessages = $this->db->query(sprintf("SELECT * FROM `messages`, `users` WHERE `messages`.`uid` = `users`.`idu` AND `messages`.`uid` IN (%s) AND `messages`.`public` = '1' AND `messages`.`page` = 0 AND `users`.`suspended` = 0 ORDER BY `id` DESC LIMIT %s", $friendslist, 25));
	...
	function sidebarTrending($bold, $per_page) {
		...
		$query = $this->db->query(sprintf("SELECT * FROM `messages`, `users` WHERE `messages`.`uid` = `users`.`idu` AND `messages`.`time` > CURRENT_DATE AND `messages`.`time` < CURRENT_DATE + INTERVAL 7 DAY AND `messages`.`tag` != '' AND `messages`.`group` = 0 AND `messages`.`public` = 1 AND `users`.`suspended` = 0"));
	...
	function getPictures() {
		...
		$query = $this->db->query(sprintf("SELECT count(`id`) FROM `messages` WHERE `type` = 'picture' AND `group` = 0 AND `uid` = '%s'", $this->profile_data['idu']));
	...
	function getLikes($start, $type, $value = null, $for = null) {
		global $LNG;
		...
		if($type == 1) {
			...
			if($start) {
				...
				while($row = $query->fetch_assoc()) {
					$rows .= '<div class="likes-users"><a href="'.$this->url.'/index.php?a=profile&u='.$row['username'].'" title="'.realName($row['username'], $row['first_name'], $row['last_name']).'" rel="loadpage"><img src="'.$this->url.'/thumb.php?src='.$row['image'].'&t=a&w=112&h=112"></a></div>';
				}
				$rows .= '</div>';
				return $rows;
			} else {
				$query = $this->db->query(sprintf("SELECT
				(SELECT count(id) FROM `likes` WHERE `post` = '%s' AND `type` = '2') as total,
				(SELECT count(id) FROM `likes` WHERE `post` = '%s' AND `type` = '2' AND CURDATE() = date(`time`)) as today,
				(SELECT count(id) FROM `likes` WHERE `post` = '%s' AND `type` = '2' AND (CURDATE()-1) = date(`time`)) as yesterday,
				(SELECT count(id) FROM `likes` WHERE `post` = '%s' AND `type` = '2' AND WEEKOFYEAR(`time`) = WEEKOFYEAR(NOW())) as this_week,
				(SELECT count(id) FROM `likes` WHERE `post` = '%s' AND `type` = '2' AND MONTH(CURDATE()) = MONTH(date(`time`)) AND YEAR(CURDATE()) = YEAR(date(`time`))) as this_month,
				(SELECT count(id) FROM `likes` WHERE `post` = '%s' AND `type` = '2' AND YEAR(CURDATE()) = YEAR(date(`time`))) as this_year", $this->page_data['id'], $this->page_data['id'], $this->page_data['id'], $this->page_data['id'], $this->page_data['id'], $this->page_data['id']));
			...
			$query = $this->db->query(sprintf("SELECT count(`likes`.`id`) FROM `likes` WHERE `likes`.`by` = '%s' AND `likes`.`type` = 2", $this->profile_data['idu']));
	...
	function getHashtags($start, $per_page, $value, $type = null) {
		...
		if($type) {
			...
			$query = sprintf("SELECT * FROM `messages`, `users` WHERE `messages`.`tag` REGEXP '[[:<:]]%s[[:>:]]' AND `messages`.`uid` = `users`.`idu` %s AND `messages`.`group` = 0 AND `messages`.`public` = 1 AND `users`.`suspended` = 0 ORDER BY `messages`.`id` DESC LIMIT %s", $this->db->real_escape_string($value), $start, ($this->per_page + 1));
	...
	function getSearch($start, $per_page, $value, $filter = null, $age = null, $type = null) {
		...
		if(($filter == 'm' || $filter == 'f' || $filter == ‘c’ || $filter == ’t’) && preg_match('/^[0-9]+-[0-9]+$/i', $age)) {
			if($filter == ‘f’) {
				$gender = 1;
			} elseif ($filter == ‘m’){
				$gender = 2;
		...
		elseif($filter == 'm' || $filter == 'f' || $filter == 'c' || $filter == 't')  {
			if($filter == 'f') {
				$gender = 1;
			} elseif ($filter == 'm'){
				$gender = 2;
		...
		if($loadmore) {
				$output .= '<div class="message-container" id="more_messages">
								<div class="load_more"><a onclick="loadPeople('.($start + $per_page).', \''.saniscape($value).'\', \''.saniscape($filter).'\', \''.saniscape($age).'\')" id="load-more">'.$LNG['view_more_messages'].'</a></div>
							</div>';
		}
		...
		if($live) {
				...
				$output .= '<div class="'.$class.'" id="group'.$row['id'].'">
								<div class="message-content">
									<div class="message-inner">
										...
										<div class="message-top">
											<div class="message-author">
												<a href="'.$this->url.'/index.php?a=group&name='.$row['name'].'" rel="loadpage">'.(empty($value) ? $row['name'] : $row['title']).'</a>
											</div>
											<div class="message-time">
											'.($row['privacy'] ? $LNG['private_group'] : $LNG['public_group']).' ('.sprintf($LNG['x_members'], $this->countGroupMembers($row['id'], 0)).')
											</div>
			...
			if($loadmore) {
					$output .= '<div class="message-container" id="'.(empty($value) ? 'more_users' : 'more_messages').'">
									<div class="load_more"><a onclick="group('.(empty($value) ? 5 : 3).', \''.htmlspecialchars($value).'\', \''.$this->db->real_escape_string($start + $this->per_page).'\', \''.$this->profile_data['idu'].'\')" id="load-more">'.$LNG['view_more_messages'].'</a></div>
								</div>';
			}
		}
		return $output;
	...
	function getPages($start = null, $value = null, $live = null) {
		...
		if(empty($value)) {
			...
			if(empty($live)) {
				...
				$class = 'message-container';
				$button = $LNG['view'];
				$link = $this->url.'/index.php?a=page&name='.$row['name'];
				$live = 0;
			}
		} else {
		...
		if($live) {
			...
			if(empty($rows)) {
				...
				foreach($rows as $row) {
					$output .= '<div class="message-inner" id="page'.$row['id'].'">
									<div class="users-button button-normal">
										<a href="'.$this->url.'/index.php?a=page&name='.$row['name'].'" rel="loadpage">'.$LNG['view'].'</a>
									</div>
									<div class="message-avatar">
										<a href="'.$this->url.'/index.php?a=page&name='.$row['name'].'" rel="loadpage">
											<img src="'.$this->url.'/thumb.php?src='.$row['image'].'&t=a&w=48&h=48">
										</a>
									</div>
									<div class="message-top">
										<div class="message-author">
											<a href="'.$this->url.'/index.php?a=page&name='.$row['name'].'" rel="loadpage">'.$row['title'].'</a>'.((!empty($row['verified'])) ? '<span class="verified-small"><img src="'.$this->url.'/'.$CONF['theme_url'].'/images/icons/verified.png" title="'.$LNG['verified_page'].'"></span>' : '').'
										</div>
										<div class="message-time">'.$LNG['page_'.$row['category']].' - '.$this->countLikes($row['id'], 2).' '.$LNG['likes'].'</div>
									</div>
								</div>';
				}
			}
				...
				$output .= '<div class="'.$class.'" id="page'.$row['id'].'">
								<div class="message-content">
									<div class="message-inner">
										...
										<div class="message-top">
											<div class="message-author">
												<a href="'.$this->url.'/index.php?a=page&name='.$row['name'].'" rel="loadpage">'.((empty($value) && is_null($live)) ? $row['name'] : $row['title']).'</a>'.((!empty($row['verified'])) ? '<span class="verified-small"><img src="'.$this->url.'/'.$CONF['theme_url'].'/images/icons/verified.png" title="'.$LNG['verified_page'].'"></span>' : '').'
											</div>
											<div class="message-time">'.$LNG['page_'.$row['category']].' - '.$this->countLikes($row['id'], 2).' '.$LNG['likes'].'</div>
										</div>
									</div>
			...
			if($loadmore) {
					$output .= '<div class="message-container" id="'.(empty($value) ? 'more_users' : 'more_messages').'">
									<div class="load_more"><a onclick="page('.(empty($value) ? 2 : 1).', \''.htmlspecialchars($value).'\', \''.$this->db->real_escape_string($start + $this->per_page).'\', \''.$this->profile_data['idu'].'\')" id="load-more">'.$LNG['view_more_messages'].'</a></div>
								</div>';
			}
		}
		return $output;
	...
	function listFriends($type = null) {
		...
		if($loadmore) {
				$output .= '<div class="message-container" id="more_messages">
								<div class="load_more"><a onclick="loadSubs('.$last.', '.$type.', '.$this->profile_data['idu'].')" id="load-more">'.$LNG['view_more_messages'].'</a></div>
							</div>';
		}
		return $output;
	}
	...
	function getActions($id, $likes = null) {
		global $LNG;
		...
		if(empty($this->id)) {
			// Output variable
			$actions = '<a href="'.$this->url.'/index.php" rel="loadpage">'.$LNG['login_to_lcs'].'</a> <div class="like_btn"> '.$likes.'</div>';
		}
	...
	function pageActivity($type, $page) {
		...
		$curr_count = $this->countLikes($page, 2);
		if($type) {
			// Update the notifications with the latest value
			$this->db->query(sprintf("UPDATE `notifications` SET `child` = '%s' WHERE `parent` = '%s' AND `from` = '%s' AND `type` = 10", $curr_count, $page, $this->id));
		} else {
			// Get the old value
			$getOld = $this->db->query(sprintf("SELECT `child` as `count` FROM `notifications` WHERE `parent` = '%s' AND `from` = '%s' AND `type` = 10", $page, $this->id));
	...
	function createPage($values, $type) {
		...
		global $LNG;
		$values['page_name'] = ($type ? $this->page_data['name'] : strtolower($values['page_name']));
		$values['page_title'] = htmlspecialchars($values['page_title']);
		$values['page_desc'] = htmlspecialchars(trim(nl2clean($values['page_desc'])));
		$values['page_phone'] = htmlspecialchars($values['page_phone']);
		$values['page_address'] = htmlspecialchars($values['page_address']);
		
		$avatar = $_FILES['page_avatar'];
		$cover = $_FILES['page_cover'];
		...
		if(!filter_var($values['page_website'], FILTER_VALIDATE_URL) && !empty($values['page_website'])) {
	...
	function deletePage($id, $type = null, $from = null) {
		...
		if($_GET['token_id'] == $_SESSION['token_id']) {
			...
			if($query->num_rows) {
				...
				// Delete the notifications (both page invite and the last message viewed)
				...
				if(!$type) {
					header("Location: ".$this->url."/index.php?a=".($from ? "admin&b=manage_pages" : "page")."&deleted=".$result['name']);
				}
			}
		}
	}
	...
	function deleteGroup($id, $type = null, $from = null) {
		...
		if($_GET['token_id'] == $_SESSION['token_id']) {
			...
			if($query->num_rows) {
				...
				if(!$type) {
					header("Location: ".$this->url."/index.php?a=".($from ? "admin&b=manage_groups" : "group")."&deleted=".$result['name']);
				}
			}
		}
	}
...
function admin_stats($db, $type, $values = null) {
	if($type == 1) {
		...
		(SELECT count(idu) FROM users WHERE CURDATE() = `date` AND `suspended` != 2) as users_today,
		(SELECT count(idu) FROM users WHERE DATE_SUB(CURDATE(), INTERVAL 1 DAY) = `date` AND `suspended` != 2) as users_yesterday,
		(SELECT count(idu) FROM users WHERE DATE_SUB(CURDATE(), INTERVAL 2 DAY) = `date` AND `suspended` != 2) as users_two_days,
		(SELECT count(idu) FROM users WHERE DATE_SUB(CURDATE(), INTERVAL 3 DAY) = `date` AND `suspended` != 2) as users_three_days,
		(SELECT count(idu) FROM users WHERE DATE_SUB(CURDATE(), INTERVAL 4 DAY) = `date` AND `suspended` != 2) as users_four_days,
		(SELECT count(idu) FROM users WHERE DATE_SUB(CURDATE(), INTERVAL 5 DAY) = `date` AND `suspended` != 2) as users_five_days,
		(SELECT count(idu) FROM users WHERE DATE_SUB(CURDATE(), INTERVAL 6 DAY) = `date` AND `suspended` != 2) as users_six_days,
		(SELECT COUNT(id) FROM pages WHERE CURDATE() = date(`time`)) AS pages_today,
		(SELECT COUNT(id) FROM pages WHERE DATE_SUB(CURDATE(), INTERVAL 1 DAY) = date(`time`)) AS pages_yesterday,
		(SELECT COUNT(id) FROM pages WHERE DATE_SUB(CURDATE(), INTERVAL 2 DAY) = date(`time`)) AS pages_two_days,
		(SELECT COUNT(id) FROM pages WHERE DATE_SUB(CURDATE(), INTERVAL 3 DAY) = date(`time`)) AS pages_three_days,
		(SELECT COUNT(id) FROM pages WHERE DATE_SUB(CURDATE(), INTERVAL 4 DAY) = date(`time`)) AS pages_four_days,
		(SELECT COUNT(id) FROM pages WHERE DATE_SUB(CURDATE(), INTERVAL 5 DAY) = date(`time`)) AS pages_five_days,
		(SELECT COUNT(id) FROM pages WHERE DATE_SUB(CURDATE(), INTERVAL 6 DAY) = date(`time`)) AS pages_six_days,
		(SELECT COUNT(id) FROM groups WHERE CURDATE() = date(`time`)) AS groups_today,
		(SELECT COUNT(id) FROM groups WHERE DATE_SUB(CURDATE(), INTERVAL 1 DAY) = date(`time`)) AS groups_yesterday,
		(SELECT COUNT(id) FROM groups WHERE DATE_SUB(CURDATE(), INTERVAL 2 DAY) = date(`time`)) AS groups_two_days,
		(SELECT COUNT(id) FROM groups WHERE DATE_SUB(CURDATE(), INTERVAL 3 DAY) = date(`time`)) AS groups_three_days,
		(SELECT COUNT(id) FROM groups WHERE DATE_SUB(CURDATE(), INTERVAL 4 DAY) = date(`time`)) AS groups_four_days,
		(SELECT COUNT(id) FROM groups WHERE DATE_SUB(CURDATE(), INTERVAL 5 DAY) = date(`time`)) AS groups_five_days,
		(SELECT COUNT(id) FROM groups WHERE DATE_SUB(CURDATE(), INTERVAL 6 DAY) = date(`time`)) AS groups_six_days,
		(SELECT COUNT(id) FROM messages WHERE `type` != 'shared' AND CURDATE() = date(`time`)) AS messages_today,
		(SELECT COUNT(id) FROM messages WHERE `type` != 'shared' AND DATE_SUB(CURDATE(), INTERVAL 1 DAY) = date(`time`)) AS messages_yesterday,
		(SELECT COUNT(id) FROM messages WHERE `type` != 'shared' AND DATE_SUB(CURDATE(), INTERVAL 2 DAY) = date(`time`)) AS messages_two_days,
		(SELECT COUNT(id) FROM messages WHERE `type` != 'shared' AND DATE_SUB(CURDATE(), INTERVAL 3 DAY) = date(`time`)) AS messages_three_days,
		(SELECT COUNT(id) FROM messages WHERE `type` != 'shared' AND DATE_SUB(CURDATE(), INTERVAL 4 DAY) = date(`time`)) AS messages_four_days,
		(SELECT COUNT(id) FROM messages WHERE `type` != 'shared' AND DATE_SUB(CURDATE(), INTERVAL 5 DAY) = date(`time`)) AS messages_five_days,
		(SELECT COUNT(id) FROM messages WHERE `type` != 'shared' AND DATE_SUB(CURDATE(), INTERVAL 6 DAY) = date(`time`)) AS messages_six_days,
		...
		(SELECT COUNT(id) FROM messages WHERE `type` = 'shared' AND DATE_SUB(CURDATE(), INTERVAL 1 DAY) = date(`time`)) AS shared_yesterday,
		(SELECT COUNT(id) FROM messages WHERE `type` = 'shared' AND DATE_SUB(CURDATE(), INTERVAL 2 DAY) = date(`time`)) AS shared_two_days,
		(SELECT COUNT(id) FROM messages WHERE `type` = 'shared' AND DATE_SUB(CURDATE(), INTERVAL 3 DAY) = date(`time`)) AS shared_three_days,
		(SELECT COUNT(id) FROM messages WHERE `type` = 'shared' AND DATE_SUB(CURDATE(), INTERVAL 4 DAY) = date(`time`)) AS shared_four_days,
		(SELECT COUNT(id) FROM messages WHERE `type` = 'shared' AND DATE_SUB(CURDATE(), INTERVAL 5 DAY) = date(`time`)) AS shared_five_days,
		(SELECT COUNT(id) FROM messages WHERE `type` = 'shared' AND DATE_SUB(CURDATE(), INTERVAL 6 DAY) = date(`time`)) AS shared_six_days,
		...
		(SELECT count(idu) FROM `users` WHERE `online` > '%s'-'%s' AND `suspended` != 2) AS online_users", time(), $values['conline']);
	} else {
		$query = "SELECT 
		(SELECT COUNT(id) FROM messages WHERE `type` != 'shared') AS messages_total,
		(SELECT COUNT(id) FROM messages WHERE `type` != 'shared' AND CURDATE() = date(`time`)) AS messages_today,
		(SELECT COUNT(id) FROM messages WHERE `type` != 'shared' AND MONTH(CURDATE()) = MONTH(date(`time`)) AND YEAR(CURDATE()) = YEAR(date(`time`))) as messages_month,
		(SELECT COUNT(id) FROM messages WHERE `type` != 'shared' AND YEAR(CURDATE()) = YEAR(date(`time`))) as messages_year,
		...
		(SELECT COUNT(id) FROM pages) AS pages_total,
		(SELECT COUNT(id) FROM pages WHERE CURDATE() = date(`time`)) AS pages_today,
		(SELECT COUNT(id) FROM pages WHERE MONTH(CURDATE()) = MONTH(date(`time`)) AND YEAR(CURDATE()) = YEAR(date(`time`))) as pages_month,
		(SELECT COUNT(id) FROM pages WHERE YEAR(CURDATE()) = YEAR(date(`time`))) as pages_year,
		...
		(SELECT count(idu) FROM users WHERE `suspended` != 2) as users_total,
		(SELECT count(idu) FROM users WHERE CURDATE() = `date` AND `suspended` != 2) as users_today,
		(SELECT count(idu) FROM users WHERE MONTH(CURDATE()) = MONTH(`date`) AND YEAR(CURDATE()) = YEAR(`date`) AND `suspended` != 2) as users_month,
		(SELECT count(idu) FROM users WHERE YEAR(CURDATE()) = YEAR(`date`) AND `suspended` != 2) as users_year,
		(SELECT count(id) FROM `likes`) as likes_total,
		(SELECT count(id) FROM `likes` WHERE CURDATE() = date(`time`)) as likes_today,
