...
function menu($user) {
	...
	if($user !== false) {
		...
		*/
		...
		return '<a href="'.$CONF['url'].'/index.php?a=welcome" rel="loadpage" title="'.$LNG['connect'].'"><div class="topbar-button">'.$LNG['connect'].'</div></a>'.$admin_url;
	}
}
function chatSmiles() {
	global $CONF;
...
class feed {
		...
		if(!$type) {
			...
			if($type) {
				...
				if($all) {
					...
					if($for == 2) {
						// Verify for the notifications page
						if(empty($likes) && empty($comments) && empty($shares) && empty($friends) && empty($chats) && empty($birthdays) && empty($pages) && empty($groups) && empty($pokes)) {
	...
	function getSearch($start, $per_page, $value, $filter = null, $age = null, $type = null) {
		...
		if(($filter == 'm' || $filter == 'f' || $filter == ÔcÕ || $filter == ÕtÕ) && preg_match('/^[0-9]+-[0-9]+$/i', $age)) {
			if($filter == ÔfÕ) {
				$gender = 1;
			} elseif ($filter == ÔmÕ){
				$gender = 2;
		...
		elseif($filter == 'm' || $filter == 'f' || $filter == ÔcÕ || $filter == ÕtÕ)  {
			if($filter == ÔfÕ) {
				$gender = 1;
			} elseif ($filter == ÔmÕ){
				$gender = 2;
